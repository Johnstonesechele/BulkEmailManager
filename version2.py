...
# -----------------------------
class BulkSMSSender:
    def __init__(self, port, baudrate=115200, timeout=5, log_cb=None):
        self.port = port
    ...
    def connect(self):
        try:
            ...
            self._log(f"Connected to GSM modem on {self.port}")
            return True
        except Exception as e:
            self._log(f"Failed to connect to GSM modem: {e}")
            return False

    def disconnect(self):
        if self.ser:
            self.ser.close()
            self._log("Disconnected from GSM modem")
    ...
    def send_sms(self, phone_number, message):
        ...
        try:
            ...
            self.ser.readline()
            self.ser.write(b'AT+CMGF=1\r')  # Set text mode
            self.ser.readline()
            self.ser.write(f'AT+CMGS="{phone_number}"\r'.encode())
            self.ser.readline()
            self.ser.write(message.encode() + b"\x1A")  # Ctrl+Z to send
            resp = self.ser.readlines()
            self._log(f"SMS sent to {phone_number}")
            return True
        except Exception as e:
            self._log(f"Failed to send SMS to {phone_number}: {e}")
            return False

    def send_bulk_sms(self, numbers, message):
        results = {"success": 0, "failed": 0}
        for num in numbers:
            if self.send_sms(num, message):
                results["success"] += 1
            else:
                results["failed"] += 1
        return results
...
class SendWorker(Thread):
    def __init__(self, df, subject, html_content, text_content, attachments, env_file,
                 jinja_strict, dkim_overrides=None):
        super().__init__()
        self.df = df
    ...
    def run(self):
        ...
        for k in list(os.environ.keys()):
            if k.startswith("SMTP_") or k.startswith("DKIM_") or k in ("SENDER_EMAIL", "EMAIL_USERNAME", "EMAIL_PASSWORD"):
...
class ContactsTab(QWidget):
    ...
    def load_csv(self):
        ...
        try:
            ...
            self.logs_cb(f"Loaded contacts: {path}")
        except Exception as e:
        ...
        try:
            ...
            if text.strip():
                if 'Segment' in df.columns:
                    df = df[df['Segment'].astype(str).str.contains(text, case=False, na=False)]
            self.df = df
            self.model = PandasModel(self.df)
            self.view.setModel(self.model)
        except Exception as e:
...
class CampaignTab(QWidget):
    def __init__(self, contacts_tab: ContactsTab, templates_tab: TemplatesTab, assets_tab: AssetsTab, logs_cb):
        ...
        env_files = sorted(f for f in os.listdir(BASE_DIR) if f.startswith(".env.") and os.path.isfile(os.path.join(BASE_DIR, f)))
        self.env_combo.addItems(env_files if env_files else [".env.it"]) 
...
class SMSTab(QWidget):
    def __init__(self, logs_cb):
        ...
        self.progress = QProgressBar()
        self.status_lbl = QLabel("Idle")
        self.contacts_edit = QPlainTextEdit()
        self.contacts_edit.setPlaceholderText("Enter phone numbers, one per line")

        layout = QVBoxLayout(self)
        layout.addWidget(QLabel("GSM Modem Port:"))
    ...
    def send_bulk_sms(self):
        ...
        if not message:
            ...
            return
        numbers = [n.strip() for n in self.contacts_edit.toPlainText().splitlines() if n.strip()]
        if not numbers:
            QMessageBox.warning(self, "No Recipients", "Please enter at least one phone number.")
            return
        self.progress.setValue(0)
        self.status_lbl.setText("Sending…")
        sender = BulkSMSSender(port, log_cb=self.logs_cb)
        sender.connect()
        results = sender.send_bulk_sms(numbers, message)
        sender.disconnect()
        self.progress.setValue(100)
        self.status_lbl.setText(f"Done. Success: {results['success']}, Failed: {results['failed']}")
        QMessageBox.information(self, "SMS Campaign Finished", f"Success: {results['success']}\nFailed: {results['failed']}")
...
class AnalyticsTab(QWidget):
    ...
    def refresh(self):
        # For demo: show log file summary, can be extended for more analytics
        log_path = LOG_FILE
        stats = {"emails_sent": 0, "emails_failed": 0, "sms_sent": 0, "sms_failed": 0}
        if os.path.exists(log_path):
            with open(log_path, 'r', encoding='utf-8') as f:
                for line in f:
                    if "Email sent to" in line:
                        stats["emails_sent"] += 1
                    if "Failed to send email" in line:
                        stats["emails_failed"] += 1
                    if "SMS sent to" in line:
                        stats["sms_sent"] += 1
                    if "Failed to send SMS" in line:
                        stats["sms_failed"] += 1
        html = f"""
        <h2>Campaign Analytics</h2>
        <ul>
            <li><b>Emails Sent:</b> {stats['emails_sent']}</li>
            <li><b>Emails Failed:</b> {stats['emails_failed']}</li>
            <li><b>SMS Sent:</b> {stats['sms_sent']}</li>
            <li><b>SMS Failed:</b> {stats['sms_failed']}</li>
        </ul>
        <p>For detailed logs, see <code>{log_path}</code></p>
        """
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.sms_tab = SMSTab(self.logs_tab.append)
        ...
        tabs.addTab(self.sms_tab, "Bulk SMS")
